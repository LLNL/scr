...
if __name__=='__main__':
  ...
  if len(sys.argv)<3 or '-h' in sys.argv[1:] or '--help' in sys.argv[1:]:
    ...
    launcher, launcher_args, run_cmd, restart_cmd, restart_args = parseargs(sys.argv[1:],launcher='lrun')
    scr_run(launcher='lrun', launcher_args=launcher_args, run_cmd=run_cmd, restart_cmd=restart_cmd, restart_args=restart_args)
