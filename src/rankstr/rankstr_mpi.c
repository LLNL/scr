...
static void* split_malloc(size_t size)
{
  if (size > 0) {
...
static void split_free(void** ptr)
{
...
{
  /* TODO: define these via configure option instead to simplify interface? */
  //int tag1 = 0;
  //int tag2 = 1;
...
void rankstr_mpi_comm_split(MPI_Comm comm, const char* str, int key, int tag1, int tag2, MPI_Comm* newcomm)
{
  ...
  /* identify our group number to be used as a color */
  int groups, groupid;
  rankstr_mpi(inputstr, comm, tag1, tag2, &groups, &groupid);

  /* use MPI_UNDEFINED for those ranks with NULL strings,
   * these procs will end up with newcomm = MPI_COMM_NULL */
  int color = groupid;
  if (str == NULL) {
    color = MPI_UNDEFINED;
  }

  /* split comm based on group id */
